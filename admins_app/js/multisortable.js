...
jQuery.fn.multiselectable = function(options) {
        ...
        if (!jQuery(t.children()).data('multiselectable'))
	{
            ...
            jQuery(t.children()).click(function(e) /* вот он главный обработчик*/
	    {
		if(!select_mlt){ select_mlt = true; return false;}	   
		//alert('ky');
                var parent = jQuery(this).parent();
                var myIndex = jQuery(parent.children())
			.index(jQuery(this));
                var prevIndex = jQuery(parent.children())
			.index(jQuery('.multiselectable-previous', parent));

                if (!e.ctrlKey)
		{
		    jQuery('.' + options.selectedClass, parent)
			    .removeClass(options.selectedClass);
		}
                if (e.shiftKey && prevIndex >= 0)
		{
                    jQuery('.multiselectable-previous', parent)
			.toggleClass(options.selectedClass);
                    if (prevIndex < myIndex) {
                        jQuery(this).prevUntil('.multiselectable-previous')
			    .toggleClass(options.selectedClass);
                    }
                    else {
                        jQuery(this).nextUntil('.multiselectable-previous')
			    .toggleClass(options.selectedClass);
                    }
                }
