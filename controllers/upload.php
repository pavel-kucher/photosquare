...
// http://php.net/manual/en/features.file-upload.errors.php
	...
	{
		$this->options = array(
			'script_url'						 => 'http://' . $_SERVER["HTTP_HOST"] . '/upload/upload_images_to_cache', /*  путь к методоу удаления картинки(этот файл) передает название */
			'upload_dir'						 => $_SERVER['DOCUMENT_ROOT'] . '/storage/uploads/', // dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/',  куда грузить
			'upload_url'						 => 'http://' . $_SERVER["HTTP_HOST"] . '/storage/uploads/', /* отвечает за путь к картинке на сайте превью итд */
			'user_dirs'							 => false,
			'mkdir_mode'						 => 0755,
			'param_name'						 => 'files',
...
// uploaded images:
				...
				''			 => array(
					'max_width'		 => $options['max_width_img'],
					'max_height'	 => $options['max_height_img'],
					'jpeg_quality'	 => 100
				),
				// Uncomment the following to create medium sized images:
				'medium'	 => array(
					'max_width'		 => $options['max_width_med'],
					'max_height'	 => $options['max_height_med'],
					'jpeg_quality'	 => 100
				),
				'previews'	 => array(
...
//'crop' => true,
					'max_width'	 => $options['max_width_prev'],
					'max_height' => $options['max_height_prev']
				)
			)
		);
		if ($options)
...
// $_FILES is a multi-dimensional array:
			...
			{
				$files[] = $this->handle_file_upload(//iconv("utf-8", "cp1251//IGNORE", $upload['name'][$index])
						$upload['tmp_name'][$index], $this->translitIt($upload['name'][$index]), $size ? $size : $upload['size'][$index], $upload['type'][$index], $upload['error'][$index], $index, $content_range
				);
			}
		} else
		{
...
class Upload extends Controller
{
	...
	/* служебные */
	/*  уникальное имя в каталоге */

	private static function get_rand_filename($path, $extension = '')
	{
		$extension	 = $extension ? '.' . $extension : '';
		$path		 = $path ? $path . '/' : '';
		do
		{
			$name	 = substr(md5(microtime()), 0, 18);
	...
	private static function scandir_all($dir)
	{
		$list = scandir($dir);
		unset($list[0], $list[1]);
	...
	private static function scandir_ex_folder($dir)
	{
		$prelist = scandir($dir);
		$list	 = array(
		);
		unset($prelist[0], $prelist[1]); /* удаляет каталоги */
		if (count($prelist) == 0)
			return;
		foreach ($prelist as $file)
		{
			if (!is_dir($dir . $file))
			{
				$list[] = $file;
	...
	private static function clear_dir_ex_dir($dir)
	{
		$list = scandir_all($dir);
	...
	/* clear_dir('./files/');     clear_dir('/var/data/files/etc/');
	 */
	...
	function upload_images_to_cache()
	{
		Auth::handleLogin();
		/*    получаем размеры изображений     */
		$options = $this->model->image_size_site();

		error_reporting(E_ALL | E_STRICT);
		$upload_handler = new UploadHandler($options); /* происходит загрузка файлов во временную директорию */
	}

	/*  следующий метод при поддтверждении копирует файлы в каталои и заносит в бд запись о файле */
	/* по умолчанию последние загруженные */

	function save_uploads_images($_id_media = -1)
	{
		Auth::handleLogin();
		/*    получаем размеры изображений     */
		/* получаем список изображений в альбоме   */
		$files_cache	 = $this->scandir_ex_folder(STORAGE_UPLOAD_IMAGE . 'previews/'); //.'previews/'
		$file_names		 = array();
		$file_ext		 = array();
		$names_files_lib = array();
		$image_size		 = array();
		$rezolution;
		foreach ($files_cache as $file)
		{
			$file_names[]	 = substr($file, 0, strrpos($file, '.'));
			$file_ext[]		 = pathinfo($file, PATHINFO_EXTENSION);
			$rezolution		 = getimagesize(STORAGE_UPLOAD_IMAGE . $file);
			$image_size[]	 = array($rezolution[0], $rezolution[1]);
		}


		$next_name	 = '';
		$count_files = sizeof($file_names);

		for ($i = 0; $i < $count_files; $i++)
		{
			$next_name			 = $this->get_rand_filename(STORAGE_PREVIEW, $file_ext[$i]);
			$names_files_lib[$i] = $next_name;
			//rename(STORAGE_UPLOAD_IMAGE . 'previews/' . $files[$i], STORAGE_PREVIEW . $names_files_lib[$i] . '.' . $file_ext[$i]); 
		}

		var_dump($file_names);
		var_dump($file_ext);
		var_dump($names_files_lib);
		var_dump($image_size);


//rename("files/log.txt", "info.txt");
//rename ("files/log.txt", "info.txt");
		/*
		  удаление файла
