<?  if (isset($this->js_edit)){ foreach ($this->js_edit as $js){ echo '<script  type="text/javascript" src="'.URL.'admins_app/'.$js.'"></script>'."\n";}}  
if (isset($this->css_edit)){ foreach ($this->css_edit as $css){ echo '<link rel="stylesheet" href="'.URL.'admins_app/'.$css.'" />'."\n";}} 
?>
		...
		<div class="col-mid-inner">
			<div class="col-top">
				<div class="col-top-inner">
					<h3>Коллекция</h3>
					<div class="buttons">
						<a class="m-btn black" href="#"><i class="icon icon-question-sign icon-white"></i></a>
						<a id="ShowPanelSettings" href="#" class="m-btn black"><i class="icon icon-cog icon-white"></i> Настройки коллекции</a>
					</div>
				</div>
			</div><!-- end .col-top -->
				...
				<div class="col-content-inner" id="col-mid">
					<div class="list list-album">
						<ul id="sortable">
							...
							<?  endforeach; ?>
							<li class="btn-add" id="-777">
								<div class="btn-add-inner">
									<a id="ShowPanelAdd" href="#"></a>
								</div>
							</li>
						</ul>
					</div><!-- end .list-album -->
					...
					<script>
						$(function() {
							$( "#sortable" )
							  .sortable({ handle: ".handle", revert: true, items: "li:not(.btn-add)", placeholder: "list-placeholder",stop: function(event, ui)
								{
									$("#message").show();
									arr = new Array();
									...
									$.post("/admin/sort_albums_in_collection", {'mas': arr, 'id_page': id_page }, function(o)
									{
										$("#message").hide();
									}),'json';
								} })
								
								.selectable({ cancel: ".btn-add" })
								.find( "li" )
						});
					</script>
			...
			<div class="modal-footer-inner">
				<div class="row-fluid">
					<div class="pull-right">
						...
						<a onclick="return save_page();" class="m-btn orange save" href="#">Сохранить настройки</a>
					</div>
				</div>
			</div>
		</div>
...
<div class="modal panel-modal add-images" id="panel-add-images">
	<div class="modal-header">
	...
	<div class="modal-body">
		...
		<div class="modal-footer">
			<div class="modal-footer-inner">
				<div class="pull-right">
					<a id="ClosePanelAdd2" class="m-btn black cansel" href="#">Отменить</a>
...
<script>
	function add_album()
	{
		...
		var id_page = $('input[name="id_doc_edit"]').val();
		$.post("/admin/sort_added_albums", {'ar_new': ar_new, 'ar_have': ar_have, 'id_page': id_page }, function(data){	if(data == 1){ $('#selectable').children().each(function(){
...
<script>
function save_page()
{	arr = new Array(); 	arr.push($('input[name=document_header_edit]').val());
	arr.push($('select[name=publish_edit]').val()); arr.push($('input[name=name_menu_edit]').val()); arr.push($('textarea[name=doc_text_edit]').val());
	arr.push($('input[name=paswd_page_edit]').val()); arr.push($('input[name=seo_title_edit]').val());	arr.push($('textarea[name=seo_description_edit]').val());
	arr.push($('textarea[name=seo_keywords_edit]').val()); arr.push($('input[name=id_doc_edit]').val()); var send = '#'+$('input[name=id_doc_edit]').val();
	send = $(send); $.post("<?=URL.'admin/save_document_edits';?>", {mas: arr}, function(data){	if(data == '1'){ send.text($('input[name=name_menu_edit]').val());}});
	return false; }
</script>
